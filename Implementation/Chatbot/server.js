...
console.log('EMBED_PROVIDER =', process.env.EMBED_PROVIDER);
console.log('CF_AI_BASE     =', process.env.CF_AI_BASE);
console.log('CF_EMBED_MODEL =', process.env.CF_EMBED_MODEL);
...
async function embedTexts(texts) {
  if (EMBED_PROVIDER === 'cf') {
    const url = `${CF_AI_BASE}/run/${CF_EMBED_MODEL}`;
    console.log('Embedding via:', url);
    ...
    if (!r.ok) throw new Error(`CF embed HTTP ${r.status} :: ${bodyText}`);
    let j;
    try { j = JSON.parse(bodyText); } catch { throw new Error(`CF embed JSON parse error :: ${bodyText}`); }
    if (!j.embeddings) throw new Error(`CF embed JSON missing 'embeddings' :: ${bodyText}`);
    return j.embeddings;
  }
...
async function vectorizeUpsert(vectors) {
  ...
  const r = await fetch(
    `https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/vectorize/v2/indexes/${CF_VECTORIZE_INDEX}/upsert`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${CF_API_TOKEN}`,
...
async function vectorizeQuery(vec, topK = 6) {
  const r = await fetch(
    `https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/vectorize/v2/indexes/${CF_VECTORIZE_INDEX}/query`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${CF_API_TOKEN}`,
